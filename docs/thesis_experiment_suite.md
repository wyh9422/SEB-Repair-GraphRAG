...
The one-shot prompt explicitly lists `selectable_ids`. Development replay exposed a protected passage outside PPR Top-30 being selected even after the instruction distinguished context-only IDs. The v3 reranker presents candidates as local aliases `D0` through `D29`, omits unselectable IDs from protected context, and retains the alias-to-passage map in its trace. This changes only presentation, not candidate membership, and never silently accepts invalid selections. Validation failures retain their specific reason.
