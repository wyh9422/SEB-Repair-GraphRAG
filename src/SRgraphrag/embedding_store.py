...
class EmbeddingStore:
    ...
    def insert_strings(self, texts: List[str]):
        """
        Insert the input list of text into the index.

        Main process:
        1. Generate a content-based stable hash ID (MDHash) for each text.
        2. Check which hash IDs already exist in the current index.
        3. Only embed the text that does not exist.
        4. Perform upsert operations on the missing records and write them to storage.

        This ensures:
        - Avoids inserting the same content repeatedly;
        - Avoids redundant embedding calculations;
        - The operation is idempotent (repeated execution has no side effects).
        """
        nodes_dict = {}
        ...
        missing_embeddings = self.embedding_model.batch_encode(texts_to_encode) #批量编码文本 → 向量（embedding）

        self._upsert(missing_ids, texts_to_encode, missing_embeddings) #将缺失的记录写入存储（upsert = update or insert）
